...
# 运行并下载数据集
get_ipython().system('wget http://labfile.oss.aliyuncs.com/courses/1081/course-5-boston.csv')
...
def mae_value(y_true, y_pred):
    n = len(y_true)
    mae = sum(np.abs(y_true - y_yred))/n
    return mae
...
# \begin{equation}
# \textrm{MSE}(y, \hat{y} ) = \frac{1}{n} \sum_{i=1}^{n} (y_i - \hat{y})^{2}\tag{14}
# \end{equation}
